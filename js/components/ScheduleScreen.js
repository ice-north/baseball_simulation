window.ScheduleScreen = ({ seasonData, setSeasonData, allTeams, selectedMonth, setSelectedMonth }) => {
  ...
  const handleProgressDate = (days) => {
    ...
    const newSeasonData = progressDate(seasonData, days);
    setSeasonData(newSeasonData);
  };

  const handleProgressToNextGame = () => {
    if (!seasonData) return;
    const newSeasonData = progressToNextGame(seasonData, 'チームA');
    setSeasonData(newSeasonData);
  };

  const handleProgressToNextPhase = () => {
    if (!seasonData) return;
    const newSeasonData = progressToNextPhase(seasonData);
    setSeasonData(newSeasonData);
  };
