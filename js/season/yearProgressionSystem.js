 ...
 */
window.processSeasonEnd = function(seasonData, allTeams) {
  const awards = {
    champion: null,           // 優勝チーム
 ...
 */
window.updateAllPlayerAges = function(allTeams) {
  const updatedTeams = {};
 ...
 */
window.checkRetirement = function(player) {
 ...
 */
window.processRetirements = function(allTeams) {
  const updatedTeams = {};
  const retirements = [];
  ...
  Object.entries(allTeams).forEach(([teamName, team]) => {
    ...
    team.players.forEach(player => {
      const { shouldRetire, hallOfFame, reason } = window.checkRetirement(player);
 ...
 * @returns {Object} - 更新されたチームデータ
 */
window.releasePlayer = function(team, playerId) {
  return {
    ...team,
 ...
 * @returns {Object} - 更新されたチームデータ
 */
window.resetSeasonStats = function(allTeams) {
  const updatedTeams = {};
 ...
 * @returns {Object} - 更新されたチームデータ
 */
window.recordAwardsToPlayers = function(allTeams, awards) {
  const updatedTeams = {};
 ...
 */
window.advanceToNextYear = function(seasonData, allTeams) {
  // 1. シーズン終了処理（表彰）
  const awards = window.processSeasonEnd(seasonData, allTeams);

  // 2. タイトルを選手に記録
  let updatedTeams = window.recordAwardsToPlayers(allTeams, awards);

  // 3. シーズン統計を通算に加算してリセット
  updatedTeams = window.resetSeasonStats(updatedTeams);

  // 4. 選手の年齢を更新
  updatedTeams = window.updateAllPlayerAges(updatedTeams);

  // 5. 引退処理
  const { updatedTeams: teamsAfterRetirement, retirements } = window.processRetirements(updatedTeams);
