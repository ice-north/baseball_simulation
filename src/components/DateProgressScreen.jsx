...
import { PHASE_INFO, formatDate, getDayOfWeek } from '../season/seasonManager.js';
...
const DateProgressScreen = ({ seasonData, setSeasonData }) => {
  ...
  const determinePitcherDecisions = (gameResult, homeTeamData, awayTeamData) => {
    ...
    if (winningStarter) {
      ...
      if (reliever) decisions.winningPitcher = reliever;
    }
  ...
  const recordPitcherDecision = (pitcher, stat, gameHome, gameAway, isHomeWin) => {
    const teamName = stat === 'losses'
      ? (isHomeWin ? gameAway : gameHome)
      : (isHomeWin ? gameHome : gameAway);
    const teamData = TEAMS_DATA[teamName];
    if (teamData) {
      const p = teamData.players.find(pl => pl.id === pitcher.id);
      if (p) {
        p.seasonStats.pitching[stat] = (p.seasonStats.pitching[stat] || 0) + 1;
        p.careerStats.pitching[stat] = (p.careerStats.pitching[stat] || 0) + 1;
      }
    }
  };
  ...
  for (let i = 0; i < firstDay; i++) calendarCells.push({ day: null, games: [] });
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const isPast = (selectedMonth < seasonData.currentDate.month) || (selectedMonth === seasonData.currentDate.month && day < seasonData.currentDate.day);
                      ...
                      <div className="space-y-1">
                        {cell.games.map((game, gIdx) => {
                          if (showAsScheduled || !game.result) {
                            return <div key={gIdx} className="text-[11px] text-yellow-300">{game.away.replace('チーム', '')} vs {game.home.replace('チーム', '')}</div>;
                          }
                          ...
                          return (
                            <div key={gIdx} className="text-[11px]">
                              <span className={awayWin ? 'text-green-400 font-bold' : 'text-gray-400'}>{game.away.replace('チーム', '')}</span>
                              <span className="text-gray-500 mx-1">{game.result.awayScore}-{game.result.homeScore}</span>
                              <span className={homeWin ? 'text-green-400 font-bold' : 'text-gray-400'}>{game.home.replace('チーム', '')}</span>
                            </div>
                          );
                        })}
                      </div>
                    )}
                    {cell.games.length === 0 && <div className="text-[10px] text-gray-500">-</div>}
                  </>
                )}
              </div>
            );
        ...
        <table className="w-full text-white">
          ...
          <tbody>
            {seasonData.standings.map((team, index) => {
              ...
              const gb = index === 0 ? '-' : ((seasonData.standings[0].wins - team.wins) - (seasonData.standings[0].losses - team.losses)) / 2;
              return (
                <tr key={team.team} className={`border-b border-gray-700 ${isUserTeam ? 'bg-blue-900/50' : ''}`}>
                  ...
                  <td className="py-3 px-3 text-center text-gray-400">{gb === '-' ? '-' : (typeof gb === 'number' ? gb.toFixed(1) : gb)}</td>
                </tr>
              );
            })}
          </tbody>
