    ...
    <div className="bg-gray-700 rounded p-3 mb-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="text-xs text-gray-200 space-y-1">
          ...
          <div className="text-gray-300 pt-1">枠内がストライクゾーン</div>
        </div>
      </div>
    </div>
  );
            ...
            <div className="flex items-center gap-1 text-sm mb-4 flex-wrap flex-shrink-0">
              {steps.map((s, i) => (
                <span key={i} className="flex items-center gap-1">
                  {i > 0 && <span className="text-gray-400 mx-0.5">&rarr;</span>}
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {detailTab === 'ability' && (<>
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              ...
              <div className="bg-gray-700 rounded p-3">
                ...
                {player.batting && (<>
                  <div className="text-xs text-gray-400 mb-1">打撃・走塁</div>
                ...
                <div className={player.batting ? 'border-t border-gray-600 mt-2 pt-2' : ''}>
                  <div className="text-xs text-gray-400 mb-1">守備</div>
              ...
              <div className="grid grid-cols-3 gap-1">
                {['pitcher', 'catcher', 'first', 'second', 'third', 'short', 'left', 'center', 'right'].map(pos => (
                  <div key={pos} className="flex items-center gap-1">
                    <span className="text-xs text-gray-300 w-8">{POSITION_NAMES[pos]?.slice(0, 2) || pos}</span>
                    <div className="flex-1 bg-gray-600 rounded h-2">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-700/50 rounded-lg p-3 flex flex-wrap items-center gap-x-4 gap-y-2">
                  <div className="text-sm">
                    <span className="text-gray-300">知名度</span>
                    <span className={`ml-2 font-bold ${fame.color}`}>{fame.text}</span>
                    <span className="ml-1 text-xs text-gray-400">({player.fame || 0})</span>
                  </div>
                  {titles.length > 0 ? (
                    <div className="flex flex-wrap items-center gap-1">
                      <span className="text-gray-300 text-sm mr-1">タイトル</span>
                      {titles.map((t, i) => (
                    ...
                    <span className="text-xs text-gray-400">タイトル獲得なし</span>
                  )}
                </div>

                {/* タイムライン */}
                {events.length === 0 ? (
                  <div className="text-gray-300 text-sm text-center py-8">経歴データがありません</div>
                ) : (
                        ...
                        <div key={i} className="relative">
                          ...
                          <div className={`rounded-lg border px-3 py-2 ${CARD[ev.color] || CARD.gray}`}>
                            <div className="flex items-center gap-2">
                              ...
                              {ev.year != null && (
                                <span className="ml-auto text-xs text-gray-300 tabular-nums">{ev.year}年目</span>
                              )}
                            </div>
                            {ev.detail && (
          ...
          {detailTab === 'stats' && (() => {
            ...
            if (history.length === 0 && (!batting.games && !pitching.games)) {
              return <div className="text-gray-300 text-sm text-center py-8">まだシーズン成績がありません</div>;
            }
            return isPitcher ? (
          ...
          {detailTab === 'abilityHistory' && (() => {
            ...
            if (!hasAbilityData) {
              return <div className="text-gray-300 text-sm text-center py-8">まだ能力値の履歴がありません（Year2以降に記録されます）</div>;
            }
