 ...
 */
export function pickTargetCell(aim) {
  if (aim === 'zone') {
    ...
    const cells = [[1, 2], [3, 2], [2, 1], [2, 3]];
    return cells[Math.floor(Math.random() * cells.length)];
  }
  if (aim === 'chase') {
    return CHASE_CELLS[Math.floor(Math.random() * CHASE_CELLS.length)];
  }
  return ZONE_CORNERS[Math.floor(Math.random() * ZONE_CORNERS.length)];
}
