...
//
// 【何のための軸か】**正解を選ばせる軸ではなく、どんな選手を作るかを選ばせる軸**。
// 「能力が低いほど一芸、高いほど全体が有利」という水準依存の正解は実測で
// 成立しなかった（下記 measured 参照）。原理そのものは進路とカテゴリ別育成
// （`growthSystem.CATEGORY_GROWTH`）の側で実装済みなので、ここは
// **色んな選手が生まれること**を担当する。
//
...
//
// ⚠ **先に試して捨てた設計**: 「能力が低いほど一芸、高いほど全体が有利」という
//   水準で正解が反転する軸にしようとしたが、育てた選手を本物のドラフトに
//   投入して測ると成立しなかった（各条件100名・4年フル・実プール5000人）:
//
...
//   高能力         16%           99%           69%
//
//   **総合点は正の重みの和なので、集中が勝つ余地が構造的に無い**。
//   係数をどう足しても「分散させたほうが総合点が高い」は変わらない。
//   ⚠ **NPBドラフト側の係数をいじって無理に成立させないこと**。
//   「弱者の兵法」は進路とカテゴリ別育成（`growthSystem.CATEGORY_GROWTH`）で
//   実装済みなので、ここでその原理を作り直さない。
//
// 実際に効くのは**目的の違い**のほう（`campTraining.TRAINING_MENUS[].serves`）:
//   守備・走塁を鍛える → 失点が減って勝率が上がる（0.673/0.682 対 0.640）
//   打撃を鍛える       → 得点が増え、ドラフト評価が上がる
// 方針の軸は「どちらの選手を作るか」を選ばせるためにある。
//
// 【選手の希望は「正解」ではない】
