...
### 高能力値の成長減衰（重要）
- **技術系(meet/power/eye/control/defense/steal)**: 70以上で減衰開始、超過1ごとに3%減衰（下限15%）
- **フィジカル系(speed/arm/stamina/bodyStamina/recovery)**: 80以上で減衰、超過1ごとに3%
- **球速**: 155km以上で減衰、超過1ごとに20%
- 技術系は69までは減衰なし、70以降徐々に減衰（80で70%, 90で40%, 100で15%）
- メイン練習の基礎成長係数は0.11（元0.167から約34%減）
- 覚醒発生率は `experience/15`（元 `experience/10` から33%減）
- **覚醒は減衰を受けない**: 発生時は +5〜14 の大幅成長（基礎成長に加算）。"何かのきっかけで飛躍する選手"を再現
