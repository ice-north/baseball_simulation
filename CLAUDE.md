...
- **ゴロは飛距離に関わらず内野手が最初に処理する**。抜けて初めて外野への安打になる
  - 旧実装は `distance < 40` のみを内野とし、40m以上のゴロを外野手の担当にしていた。
    ゴロの飛距離は `15 + 初速×0.15 + 乱数20` で大半が40mを超えるため、
    **内野への打球が全体の10.5%しか発生せず、遊撃・二塁の守備力がほぼ無意味だった**
- **中堅の担当範囲は左右両翼より広い**（基準±21度）。旧実装は±15度で両翼の方が広かった
- 実測分布: 中堅29.8% / 右翼23.2% / 左翼15.6% / 一塁10.0% / 遊撃8.5% / 三塁6.8% / 二塁6.1%
  （内野31.4% / 外野68.6%。左右翼の差は左打者が55%と多いため）
- **ゴロの捕球率は「正面0.83 / 横0.60−難易度×0.30」**。全ゴロに適用するため旧値(0.97/0.88)から
  引き下げて較正した（旧値のままだとリーグ打率が.194まで落ちる）
- 効果: 内野守備35→11.9失点/試合, 50→10.2, 65→9.2, 80→8.4
